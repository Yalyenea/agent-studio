use gpui::{
    App, AppContext, Context, Entity, FocusHandle, Focusable, IntoElement, ParentElement, Pixels, Render, Styled, Window, px
};
...
use crate::{
    AgentMessage, AgentMessageContent, AgentMessageData,
    AgentTodoList,
    MessageContent, PlanEntry, PlanEntryPriority, PlanEntryStatus, ResourceContent,
    ToolCallData, ToolCallItem, ToolCallKind, ToolCallStatus, ToolCallContent,
    UserMessage, UserMessageData,
};

pub struct ConversationStory {
    focus_handle: FocusHandle,
}

impl super::Story for ConversationStory {
    fn title() -> &'static str {
        "Conversation"
    }
...
impl Focusable for ConversationStory {
    fn focus_handle(&self, _: &App) -> FocusHandle {
        self.focus_handle.clone()
    }
}

impl Render for ConversationStory {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let todo_list = AgentTodoList::new()
        ...
        let tool_call1 = ToolCallItem::new(
            ...
            ToolCallData::new("call_001", "Reading auth.rs")
                ...
                .with_content(vec![
                    ToolCallContent::new("Successfully read auth.rs (45 lines)")
                ])
        ).open(false);
